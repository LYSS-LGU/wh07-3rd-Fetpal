...
graph TB
    subgraph UserLayer["👥 사용자 레이어"]
        style UserLayer fill:#E3F2FD,stroke:#1976D2,stroke-width:5px
        User1["<b>🖥️ 데스크톱<br/>사용자</b>"]
        User2["<b>📱 모바일<br/>사용자</b>"]
    end

    subgraph FrontendLayer["🌐 프론트엔드 레이어"]
        style FrontendLayer fill:#F3E5F5,stroke:#7B1FA2,stroke-width:5px

        direction TB
        NextJS["<b>Next.js 14</b><br/>App Router<br/>TypeScript 5.x"]

        NextJS --> HookComp["<b>Hook Composition</b><br/>60% 코드 감소"]
        NextJS --> CSSMod["<b>CSS Modules</b><br/>Co-location<br/>BEM 네이밍"]
    end

    subgraph BackendLayer["☁️ 백엔드 레이어<br/>Supabase BaaS"]
        style BackendLayer fill:#E0F2F1,stroke:#00796B,stroke-width:5px

        direction TB
        Auth["<b>🔐 Supabase Auth</b><br/>JWT 토큰 인증"]
        DB["<b>🐘 PostgreSQL 15.x</b><br/>40개+ 테이블<br/>pgvector"]
        Storage["<b>📦 Storage</b><br/>S3 호환"]
        Realtime["<b>⚡ Realtime</b><br/>WebSocket"]
        RLS["<b>🔒 RLS</b><br/>47개 정책"]
    end

    subgraph AILayer["🤖 AI 서버 레이어<br/>FastAPI"]
        style AILayer fill:#FCE4EC,stroke:#C2185B,stroke-width:5px

        direction TB
        FastAPI["<b>FastAPI Server</b><br/>Python 3.10<br/>GPU 가속"]

        YOLO["<b>YOLO 3종 모델</b><br/>Health/Eyes/Skin<br/>mAP50: 88.2%"]
        RAGSys["<b>RAG System</b><br/>pgvector 검색<br/>384차원 임베딩"]

        FastAPI --> YOLO
        FastAPI --> RAGSys
    end

    subgraph ExternalLayer["🌍 외부 API 레이어"]
        style ExternalLayer fill:#FFF3E0,stroke:#F57C00,stroke-width:5px

        direction TB
        KakaoMap["<b>🗺️ Kakao Map</b><br/>병원/약국 검색"]
        LLMs["<b>💬 Multi-LLM</b><br/>GPT-4/Gemini<br/>Claude"]
    end
